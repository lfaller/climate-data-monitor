...
class QuiltPackager:
    """Create and manage Quilt packages for climate data."""
    ...
    def push_to_registry(self, pkg: quilt3.Package) -> bool:
        ...
        if not self.push_enabled:
            ...
            try:
                ...
                pkg.build(
                    ...
                    message=f"Climate data release {self.package_name}",
                )
                logger.info(
        ...
        try:
            ...
            pkg.push(
                ...
                message=f"Automated climate data release {self.package_name}",
            )
    ...
    def _generate_metadata_from_report(self, report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Add climate-specific metrics if present
        if "temperature_range" in report:
            metadata["temperature_range"] = report.get("temperature_range")
        if "precipitation_stats" in report:
            metadata["precipitation_stats"] = report.get("precipitation_stats")
        if "station_count" in report:
            metadata["station_count"] = report.get("station_count")
    ...
    def _extract_version_info(self, filename: str) -> str:
        ...
        """
        # Try to extract date from filename (e.g., climate_data_2025-11-21.csv)
        import re
    ...
    def full_package_workflow(
        ...
        Args:
            data_file: Path to climate data file
